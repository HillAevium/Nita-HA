...
class NitaProd{
	...
	private $eol="\n";
	public 	$SesNames		= array("Cart"=>"ShoppingCart"); // Session Key for our User
	
	public 	$ERROR			=	false;
	private $HTTP				=	"http://10.137.1.49:6652/"; // URL to our SOAP client
	public 	$URLS				=	array('programs' => 'ProgramService.svc?wsdl'); // WSDL for our programs module
	...
	function __construct(){
		...
		$this->TEMP['bill']				="billing_form.tpl";	// Our billing form
	}
				...
				if($contBuff=$this->LoadTemplate($this->rp.$this->TEMPPath.$this->TEMP['pListCont'])){ // Get our product list template from the server
					...
					if($itemBuff=$this->LoadTemplate($this->rp.$this->TEMPPath.$this->TEMP['pList'])){// Get Item templace from the server
						if(intval($this->Limit)>0){ 
							if(isset($_GET['pg'])) $this->Page=intval($_GET['pg']);
	...
	function Billing(){
		if($contBuff=$this->LoadTemplate($this->rp.$this->TEMPPath.$this->TEMP['bill'])){
			$this->HTML=$contBuff;
		}
	}
	// ************************************************************
	...
	function changeCart($redirt=true){ // Changes the quantities of items in the cart.	
		if(!isset($_POST['Key']) || !isset($_POST['Qty'])){
			$GoTo = $this->Files['Cart']; 
		...
		foreach($keys as $k => $v){ // For Each Item go through the system and update the quatities
			...
			if($qtys[$k]< ( count($this->Cart[$v]["attnd"])-1) ) $this->Cart[$v]["qty"] = $qtys[$k] = count($this->Cart[$v]["attnd"])-1;
			/*
		...
		switch($_POST['Controller']){
			...
			default: $GoTo = $this->Files['Cart']; break;
		}
		if($redirt) header(sprintf("Location: %s", $GoTo));
	}
	...
	function AttenDropList($Key, $Cnt, $Default=false){
		$AttnBuff='<div id="attndHldr_'.$Key.'_'.($Cnt+1).'">
					<select name="attnd_'.$Key.'[]" id="attnd_'.$Key.'_'.($Cnt+1).'">';
		$AttnBuff.='<option value="0" title="Select Attendee">Default Attendee</option>';
		...
		$AttnBuff.='</select>';
		if($Default!==false && $Default!=0) $AttnBuff.='&nbsp;<a href="#" id="RmvAttnd_'.$Key.'_'.($Cnt+1).'" onclick="RmvElem('.$Key.','.($Cnt+1).'); return false;">--</a>';
		$AttnBuff.='</div>';
		return $AttnBuff;
	}
	function displayCart(){
		...
		require_once($this->rp.'includes/NitaUser.php'); // Lets include our User Class
		$User = new NitaUser(); // Initial the user class
		if($User->Authenticated){
			var_dump($User->Auth);
		}
		
		if(isset($_POST['Controller']) && $_POST['Controller'] == "CkOut"){ 
			if($User->Authenticated){
				// If we are authenticated we need to go to the billing form
				echo 'Go to billing';
			} else header(sprintf("Location: %s", $this->Files['Auth'])); // If we have not authenticated go to the log-in form
		}
		...
		$CartButton = '<div class="BtnContinue" id="CstmBtn"><a href="#" title="Continue Shopping">Continue Shopping</a></div>
		...
		try{
			if($contBuff=$this->LoadTemplate($this->rp.$this->TEMPPath.$this->TEMP['cList'])){
						...
						if($Item['qty']>0){
							...
							if($Item['type']==1){ // If we are looking at a program add our attendee list
								...
								if(count($Attn)<$Item['qty']){ // If our selected attendees is less than our program quanity add another selection box
									while(count($Attn)<$Item['qty']) $Attn[]=$this->AttenDropList($Key, count($Attn)); // Add another selection box based on the number we are short
								} else if($Item['attnd'][count($Item['attnd'])-1] != 0) $Attn[]=$this->AttenDropList($Key, count($Attn)); // If the last attendee is not 0 then add another selection for more selection.  Will use jQuery to add another selection box and update the quantity
								$Attn=implode("",$Attn);
								
							} else $Attn='';
							$myBuff.=$this->processCodes($itemBuff,array('ID'				=> $Item['id'],
																		 'Qty'			=> '<input type="text" name="Qty[]" id="Qty_'.$Key.'" value="'.$Item['qty'].'"'.(($Item['type']==1)?' readonly="readonly"':'').'>',
																		 'Name'			=> $Item['name'],
				...
				$this->HTML.=$this->processCodes($contBuff, array('CartUrl'				=> $_SERVER['PHP_SELF'],
																  ...
																  'Total'				=> '<input type="input" name="Cart_Total" id="Cart_Total" disabled="disabled" value="$'.number_format($this->Totals['Total'],2,'.',',').'">',
																  'Tax'					=> '<input type="input" name="Cart_Tax" id="Cart_Tax" disabled="disabled" value="$'.number_format($this->Totals['Tax'],2,'.',',').'">',
																  'Ship'				=> '<input type="input" name="Cart_Ship" id="Cart_Ship" disabled="disabled" value="$'.number_format($this->Totals['Ship'],2,'.',',').'">',
																  'Grand'				=> '<input type="input" name="Cart_Grand" id="Cart_Grand" disabled="disabled" value="$'.number_format($this->Totals['Grand'],2,'.',',').'">'
																  )); // Place our cart informaiton into our template
				ob_start(); ?>
<script type="text/javascript">
$(document).ready(function (){ var Attns = $("#cartForm select[id^='attnd_']"); var Key=null;
		$("#cartForm select[id^='attnd_']").change( function(){ UpdateCart() } );
		for(var n=0; n<Attns.length; n++){ var id=Attns[n].id.split("_");
			if(Key!=id[1]){ Key=id[1]; var Attn = $("#cartForm select[id^='attnd_"+Key+"_']:last"); $(Attn[0]).change( function() { CloneElem($(this)) } );
			} } 
		$('.BtnContinue a').click(function() {$('#Controller').val('CtnShp'); $('#cartForm').submit(); return false; });
		$('.BtnCheckOut a').click(function() {$('#Controller').val('CkOut'); $('#cartForm').submit(); return false; });
		$('.BtnRemove a').click(function() { RmvItem($(this)); return false; });
	});
function RmvItem(Elem){ var id=$(Elem).parent().parent().children("input[id^='Key_']").val();
	$("#cartForm #Qty_"+id).val(0);	$("#cartForm div[id^='attndHldr_"+id+"_']").remove();
	$(Elem).parent().parent().parent().css("display","none"); UpdateCart();
}
function CloneElem(Elem){ var id=$(Elem).attr("id").split("_"); var CLONE = $(Elem).parent().clone();
	$(CLONE).attr("id", 'attndHldr_'+id[1]+'_'+(parseInt(id[2])+1)); $(CLONE).children("select").val(0);
	$(CLONE).children("select").attr("id", 'attnd_'+id[1]+'_'+(parseInt(id[2])+1));
	$(CLONE).children("select").change( function() { CloneElem($(this)) } ); $(CLONE).appendTo( $(Elem).parent().parent() );
	$(Elem).unbind('change'); $(Elem).change( function(){ UpdateCart() } );
	$(Elem).parent().append('&nbsp;<a href="#" id="RmvAttnd_'+id[1]+'_'+(parseInt(id[2]))+'" onclick="RmvElem('+(parseInt(id[1]))+','+(parseInt(id[2]))+'); return false;">--</a>');
	var QTY = $("#cartForm input[id='Qty_"+id[1]+"'");
	if( ($(Elem).parent().parent().children("div").children("select").length-1)>parseInt($(QTY).val) ) $(QTY).val(parseInt($(QTY).val())+1);
	UpdateCart(); }
function RmvElem(KEY, ID){ $('#attndHldr_'+KEY+'_'+ID).remove();
	var QTY = $("#cartForm input[id='Qty_"+KEY+"'");
	$(QTY).val(parseInt($(QTY).val())-1); UpdateCart();
}
function UpdateCart(){
	var options = { 
    ...
    success:	showResponse,  // post-submit callback 
		dataType:	'xml', 
    url:  		'/cart/update.php',       // override for form's 'action' attribute 
    type:			'post'       // 'get' or 'post', override for form's 'method' attribute 
  };
	$('#cartForm').ajaxSubmit(options); 
}
function showRequest(formData, jqForm, options) {  // Debugging function
  var queryString = $.param(formData); 
 	alert('About to submit: \n\n' + queryString); 
	return true; 
} 
function showResponse(responseXML) { 
	alert('status: ' + $(responseXML).text() );
	//alert(responseXML);
}
</script>
<? $this->HTML.=ob_get_contents(); ob_end_clean();
	...
	public function Auth(){// Authinticate our user from the cart
		require_once $this->rp.'includes/NitaUser.php';
		$User = new NitaUser(); // Initial the user class
		//var_dump($User->Auth);
		
		if( $User->LogIn() ) header(sprintf("Location: %s", $this->Files['Cart'])); // If user is authenticated go back to our cart to select attendees
		else $this->HTML=$User->HTML;
	}
	// ************************************************************
	// ***                      Pagination                      ***
	// ************************************************************
	function Pagination(){ // return a string for pagination
		$BUFF='';
		$QString='';
		if (!empty($_SERVER['QUERY_STRING'])) $QString = $this->QueryReplace('pg','');
		$BaseString = $_SERVER['PHP_SELF']."?".$QString.((strlen(trim($QString))>0)?"&":"")."pg=";
